...
for iteration,emd in enumerate(emd_values_arr):
    ...
    plt.errorbar(x=bin_centers+off, y=median, yerr=[one_sigma_loe,one_sigma_hie], label=labels[iteration],solid_capstyle = 'butt')
    
plt.legend(loc='upper right')
plt.xlabel(r'$\eta$')
plt.ylabel('EMD')
plt.savefig('/ecoderemdvol/ttbar_egun_epgun_ae_comp_eta_EMD.pdf',dpi=600)
