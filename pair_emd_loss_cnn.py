...
class pair_EMD_CNN:
    ...
    def ittrain(self,calQ_data,num_filt, kernel_size, num_dens_neurons, num_dens_layers, num_conv_2d, num_epochs,Loss):
        ...
        fig=plt.figure()
        fig=plt.hist(y_val, alpha=0.5, bins=np.arange(0, 15, 0.08), label='TrueEMD')
        fig=plt.hist(y_val_preds, alpha=0.5, bins=np.arange(0, 15, 0.08), label='EMDCNN')
        ...
        x_bins = np.arange(0, 15, 0.08)
        y_bins = np.arange(0, 15, 0.08)
